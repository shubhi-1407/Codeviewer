...
class SnippetForm(forms.ModelForm):
    snippet_tag=forms.CharField(required=False,widget=forms.TextInput(attrs={'class':'selectpicker form-control','placeholder':'enter tag(optional)'}))

    class Meta:
        model = Snippet
        fields = ('orig_code', 'language', 'expiration', 'exposure', 'title', )
        widgets = {
            'orig_code': forms.Textarea(attrs={'class': 'form-control', 'rows': '10',
                                                        'spellcheck': 'false'}),
    ...
    def save(self, request):
        ...
        snippet.save()
        tag_list=[t.strip().lower() for t in self.cleaned_data['snippet_tag'].split(',') if t]
        if len(tag_list)>0:
...
class ContactForm(forms.Form):
    ...
    NEW_FEATURE='nf'
    OTHER='o'
    purpose_choices=((BUG,'Bug'),(FEEDBACK,'Feedback'),(NEW_FEATURE,'Feature Request'),(OTHER,'Other'),)
...
class CreateUserForm(UserCreationForm):
    ...
    def save(self,request):
        ...
        subject=render_to_string('codeviewer/email/activation_subject.txt',context)
        email=render_to_string('codeviewer/email/activation_email.txt',context)
